...
const ApplicantScoring = () => {
  ...
  const handleScoreChange = (person, field, value) => {
    ...
    setPersons((prev) =>
      prev.map((p) =>
        p.person_id === person.person_id ? { ...p, [field]: numericValue } : p,
      ),
    );
  };
    ...
    return {
      ...
      final_rating:
        (Number(scores.english ?? person.english ?? 0) +
          ...
          Number(scores.abstract ?? person.abstract ?? 0)) /
        5,
  ...
  const saveSingleRow = async (person) => {
    try {
      ...
      const normalized = {
        ...
        english:
          saved.english != null
            ? Number(saved.english)
            : Number(person.english || 0),
        science:
          saved.science != null
            ? Number(saved.science)
            : Number(person.science || 0),
        filipino:
          saved.filipino != null
            ? Number(saved.filipino)
            : Number(person.filipino || 0),
        math:
          saved.math != null ? Number(saved.math) : Number(person.math || 0),
        abstract:
          saved.abstract != null
            ? Number(saved.abstract)
            : Number(person.abstract || 0),
        final_rating:
          saved.final_rating != null
            ...
            : (Number(saved.english ?? person.english ?? 0) +
                ...
                Number(saved.abstract ?? person.abstract ?? 0)) /
              5,
      ...
      setPersons((prev) =>
        prev.map((p) =>
          p.person_id === normalized.person_id ? { ...p, ...normalized } : p,
        ),
      );
            ...
            {currentPersons.map((person, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* SCORE INPUTS */}
                  {["english", "science", "filipino", "math", "abstract"].map(
                    (field) => (
                      <TableCell
                        sx={{
                          color: "black",
                      ...
                      >
                        <TextField
                          value={
                            editScores[person.person_id]?.[field] ??
                            person[field] ??
                            0
                          }
                          onChange={(e) =>
                            handleScoreChange(person, field, e.target.value)
                          }
                          size="small"
                          type="number"
                          sx={{ width: 70 }}
